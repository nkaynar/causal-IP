randomGraph<-function(n,restrict=c('acyclic'),pedge=1/(n-1), topology="random") {
  ...
  if ( !any(restrict=="sufficient")) {
    ...
    M<-randomGraph(n,union(restrict,"sufficient"),pedge=0.1)
    
    #draw a sufficient model for the covariances
    Me<-randomGraph(n,union(restrict,"sufficient"),pedge=0.2)
    ...
    M$Ce<-A%*%Me$Ce%*%t(A)+diag( 0.1+ abs(0.02*rnorm(n)) )
    
    #remember to update M$Ge
    M$Ge<-abs(M$Ce) > 0.9  #replace also Ge which includes the true graph
    M$Ce[abs(M$Ce) < 1e-10] <-0
    diag(M$Ge)<-0
    
    M$G <- data.matrix(read.csv(file = 'D.csv'))
    M$Ge <- data.matrix(read.csv(file = 'E.csv'))
    #write.csv(M$G,"/home/umit/Documents/Nur/ASP/hyttinen2014uai_ver6/pkg/R/D.csv", row.names = FALSE)
    #write.csv(M$Ge,"/home/umit/Documents/Nur/ASP/hyttinen2014uai_ver6/pkg/R/E.csv", row.names = FALSE)
  ...
  if ( topology=='random') {
    ...
    if ( any( restrict == 'acyclic') ) {
      ...
      M$G<-data.matrix(read.csv(file = 'E.csv'))
