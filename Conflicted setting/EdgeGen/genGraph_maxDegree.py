...
def GenRandomGraph_maxdegree(noVer,max_degree):
   ...
   # while not enter or not enter2:
    while not enter:
        ...
        selected_edges = []
        select_nodes_1 = random.sample(vertices, int(noVer/2))
    ...
    pd.DataFrame(D_true_ASP).to_csv("../ASP/hyttinen2014uai_ver6/pkg/R/D.csv",  index=None)  
    pd.DataFrame(E_true_ASP).to_csv("../ASP/hyttinen2014uai_ver6/pkg/R/E.csv",  index=None)  
    
    
    pd.DataFrame(D_true_ASP).to_csv("../ASP_conflict_IP/hyttinen2014uai_ver6/pkg/R/D.csv",  index=None)  
    pd.DataFrame(E_true_ASP).to_csv("../ASP_conflict_IP/hyttinen2014uai_ver6/pkg/R/E.csv",  index=None)  
