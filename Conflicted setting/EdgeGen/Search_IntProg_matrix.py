...
def MainintProg_searchmatrix(S, C,w_s,w_c, V, path_con, path_edge, path_length, edges_in, edges_used_index, prev_sol, previous_graph_edge_indices):
   ...
   # mod.setParam('TimeLimit', 600)
    ...
    # bak = np.load('edges_ind.npy').tolist()  
    # A_dum = np.zeros(len(vars_tup1),dtype=int)
    # for ii in bak:
    #     A_dum[ii] = 1
    #     #no confounder
    #     #if all_edges[e][1]==-3:
    ...
    A11 = 100000*np.ones(len(vars_tup4),dtype=int) 
    #mod.addConstr( A11 @ z_s == 0)  
    w_c_all = np.array(w_c_all)
    w_s_all = np.array(w_s_all)
    
    A12 = 0.001*np.ones(len(vars_tup1),dtype=int) 
    #mod.setObjective(A10 @ z+ A11 @ z_s+ A12@x, GRB.MINIMIZE)
    mod.setObjective(w_c_all@ z+ w_s_all @ z_s, GRB.MINIMIZE)
    #mod.setObjective(A10 @ z, GRB.MINIMIZE)
    #mod.setObjective(LinExpr(coef_obj,vars_obj), GRB.MINIMIZE)   
    #A9 = np.ones(len(vars_tup1),dtype=int)  
    #mod.setObjective(A9 @ x, GRB.MAXIMIZE)      
