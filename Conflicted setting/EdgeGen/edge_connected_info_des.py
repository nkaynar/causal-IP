...
# -*- coding: utf-8 -*-
"""
Generate paths considering descendancy relations
...
def con_edge_disc_des(D_true,E_true, dum_verts, noVer,maxLength,timelimit):    
    ...
    for pair_no, comb_1 in enumerate(pairs): #pair_no give the index of the current pair, which is comb_1
     ...
     if toc_end-tic_start>timelimit:
       ...
       # paths_dict['%s_%s'%(comb_1[0],comb_1[1])] = d_paths
        ...
        path_collider['%s_%s'%tuple(sorted([i,j]))] = this_collider 
        path_noncollider['%s_%s'%tuple(sorted([i,j]))] = this_noncollider
        path_directed['%s_%s'%tuple(sorted([i,j]))] = this_dirpath
        path_withcollider['%s_%s'%tuple(sorted([i,j]))] = there_is_collider
    ...
    for i in range(noVer):
        for j in range(noVer):
            if i<j:
                path_dir_index['%s_%s'%(i,j)] = [t_ind for t_ind, t_el in enumerate (path_directed['%s_%s'%tuple(sorted([i,j]))]) if t_el==1 ]
    ...
    for i in range(noVer):
        for j in range(i+1,noVer):
            path_col_index['%s_%s'%(i,j)] = [t_ind for t_ind, t_el in enumerate (path_withcollider['%s_%s'%tuple(sorted([i,j]))]) if t_el==1 ]
    ...
    for pair_no, comb_1 in enumerate(pairs): 
        ...
        if toc_end-tic_start<timelimit:
            i = comb_1[0]
            j = comb_1[1]       
            for i_n in path_col_index['%s_%s'%tuple(sorted([i,j]))]:                       
                des_comb = [] 
                dsep_info = np.zeros((len(c_subsets['%s_%s'%tuple(sorted([i,j]))])),dtype=int)
                colliders = path_collider['%s_%s'%tuple(sorted([i,j]))][i_n]
                noncolliders = path_noncollider['%s_%s'%tuple(sorted([i,j]))][i_n] 
                this_des = tuple([des_store[ii] for ii in colliders])
                paths_cover = []
                ...
                for i_p in range(len(this_path_cover_combo)):
                    edge_info = path_edge['%s_%s'%tuple(sorted([i,j]))][i_n].copy()
                    con_info = path_con['%s_%s'%tuple(sorted([i,j]))][i_n].copy()
                    col_subsets = set()
                    ...
                    for c_no in path_des_req['%s_%s'%tuple(sorted([i,j]))][i_n]:
                        c_set = c_subsets['%s_%s'%tuple(sorted([i,j]))][c_no]                    
                        if all(bool(set(this_ss) & set(c_set)) for this_ss in col_subsets ):
                            con_info[c_no] = 1
                              
                           
                    #if '%s_%s'%tuple(sorted([i,j])) in path_con_des.keys():
                    path_collider['%s_%s'%tuple(sorted([i,j]))].append(colliders)
                    path_noncollider['%s_%s'%tuple(sorted([i,j]))].append(noncolliders)
                    path_des_req['%s_%s'%tuple(sorted([i,j]))].append([])
                    path_con['%s_%s'%tuple(sorted([i,j]))].append(con_info)
                    path_edge['%s_%s'%tuple(sorted([i,j]))].append(edge_info)
                    path_length['%s_%s'%tuple(sorted([i,j]))].append([sum(edge_info)])
                    #else:
                      #   path_con_des['%s_%s'%tuple(sorted([i,j]))] = [con_info]
                      #   path_edge_des['%s_%s'%tuple(sorted([i,j]))] =[edge_info]
                      #   path_length_des['%s_%s'%tuple(sorted([i,j]))] = [[sum(edge_info)]]
    ...
    #return(path_con,path_edge, path_length,path_dir_index,path_col_index,path_collider,path_noncollider,path_des_req) 
    return(path_con,path_edge, path_length) 
