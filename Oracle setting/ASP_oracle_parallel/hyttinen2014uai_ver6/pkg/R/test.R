test<-function(algo='log-weights', n, n_i, n_f) {
  ...
  #run the inference
  L<-pipeline(n=n, n_i=n_i, n_f=n_f,
              exconf=exconf,schedule=schedule,
              test=test,p=p,solver=solver,
              encode=encode,weight=weight,
              N=N,restrict=restrict)
  return(L)
  #write.table(L$solving_time, "test_val.txt")
}