...
def MainintProg_searchmatrix(S, C,w_s,w_c, V, path_con, path_edge, path_length, edges_in, edges_used_index, prev_sol, previous_graph_edge_indices, path_IV_violater, validIV,it_no):
    ...
    y = mod.addMVar(len(vars_tup2),vtype=GRB.CONTINUOUS,name="y")
    ...
    A_edu = np.zeros(len(vars_tup1),dtype=int)
    A_edu[21] = 1
    #A_edu[1] = 1
    mod.addConstr(A_edu @ x == 1)
    
    A_prior = np.zeros(len(vars_tup1),dtype=int)
    A_prior[1] = 1 # edu -> prox
    A_prior[3] = 1 # prox -> south66
    A_prior[6] = 1 # prox -> smsa66
    A_prior[31] = 1 # wage -> south66
    A_prior[37] = 1 # wage -> smsa66
    A_prior[10] = 1 # wage -> prox

    
    for e in vars_tup1:
        if (all_edges[e][0]==5 and all_edges[e][1]==-2) or (all_edges[e][2]==5 and all_edges[e][1]==-1): #race cannot have incoming edges
            A_prior[e] = 1 
    mod.addConstr(A_prior @ x == 0)
    ...
    if validIV:
        ...
        #pdb.set_trace()
        A_prox_edu = np.zeros((1,len(vars_tup1)))
        A_prox_edu[0,0] = 1
        A_prox_edu[0,1] = 1
        A_prox_edu[0,2] = 1
        mod.addConstr(A_prox_edu @ x  >= 1)  
    ...
    if mod.status == 3: #this never happens
