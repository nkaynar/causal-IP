...
def con_edge_disc_lim(D_true,E_true, dum_verts, noVer,maxLength,IV_cov):    
       ...
       # paths_dict['%s_%s'%(comb_1[0],comb_1[1])] = d_paths
            ...
            for d_p in range(len( d_paths)):
              ...
              # Conditioning set doesn't include any of the nodes in current pair. 
                ...
                if i==0 and j==4:
                    cov_ind = c_subsets['%s_%s'%tuple(sorted([i,j]))].index(IV_cov)
                    
                    if dsep_info[cov_ind] == 1 and 21 not in edges_in_path_ind:
                        this_IV_violating = 1
                    else:
                        this_IV_violating = 0
                        
                elif i in IV_cov and j==4: #(this implies i<7)    
                 
                    if if_dir_paths(current_path)==2:
                        this_IV_violating = 1
                    else:
                        this_IV_violating = 0
                elif i==4 and j in IV_cov: #this implies j>7
