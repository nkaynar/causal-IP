test.BIC<-function(vars,C,test_data,verbose=FALSE) {
  ...
  pbic11 <- l1 - k1/(2)*log(nrow(X)) #*test_data$p_threshold
  pbic21 <- l2 - k2/(2)*log(nrow(X)) #*test_data$p_threshold
  
  
  pbic1 <- log(pbic11/(pbic11+pbic21))
  pbic2 <- log(pbic21/(pbic11+pbic21))
  ...
  test_result$independent <- ( logp[1] > logp[2] )
